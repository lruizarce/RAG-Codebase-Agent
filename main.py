...
from backend.utils.chunking_parsing_AST import parse_repo_store_all
from backend.utils.embeddings import get_huggingface_embeddings
from backend.utils.pinecone_interactions import (
    store_embeddings,
    embedding_model,
    pinecone_index,
)
...
def perform_rag(query: str, namespace="default") -> str:
    ...
    vector = (
        ...
        else raw_query_embedding.tolist()
    )
...
async def query_codebase(request: QueryRequest):
    try:
        ...
        query = request.query
        history_context = "\n".join(
            [f"{entry['role']}: {entry['content']}" for entry in request.history]
        )
        augmented_query = (
            f"History:\n{history_context}\n\nQuery:\n{query}"
            if history_context
