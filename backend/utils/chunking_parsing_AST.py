...
class SimpleTreeSitterParser:
    ...
    def parse(self, code: str) -> List[Dict]:
        ...
        for child in root.children:
            logger.debug(
                f"Node Type: {child.type}, Start: {child.start_point}, End: {child.end_point}"
            )
        ...
        for file in files:
            ...
            if extension in SUPPORTED_EXTENSIONS:
                ...
                try:
                    ...
                    all_chunks.append(
                        {"type": "file", "content": code, "file_path": file_path}
                    )
                except ValueError as ve:
                    logger.error(
                        f"Language not supported or parsing error for {file_path}: {ve}"
                    )
                    all_chunks.append(
                        {"type": "file", "content": code, "file_path": file_path}
                    )
