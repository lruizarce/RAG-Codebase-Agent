...
from utils.chunking_parsing_AST import parse_repo_store_all
from utils.embeddings import get_huggingface_embeddings
from utils.pinecone_interactions import (
    store_embeddings,
    embedding_model,
    pinecone_index,
)
...
PINECONE_INDEX_KEY = "codebase-rag"
...
async def submit_repo(request: RepoRequest):
    ...
    try:
        ...
        if not chunks:
            raise HTTPException(
                status_code=400, detail="No valid code chunks found in the repository."
            )

        # Generate embeddings
        embeddings = get_huggingface_embeddings(
            [str(chunk) if isinstance(chunk, dict) else chunk for chunk in chunks]
        )
        ...
        for chunk in chunks:
            ...
            documents.append(
                Document(page_content=content, metadata={"repo_url": request.repo_url})
            )
